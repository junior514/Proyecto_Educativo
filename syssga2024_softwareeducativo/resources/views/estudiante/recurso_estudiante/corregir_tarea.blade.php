...
@section('contenido')
    ...
    <div class="row mt-2">
        <div class="col-md-12">
            ...
            @endphp
            <form action="{{ route('recurso_estudiante.update', $entregaTarea->idEntregaTarea) }}" method="post"
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="file" accept=".pdf, .doc, .docx" class="form-control"
                                        name="archivoEntega" {{ $isHora ? '' : 'disabled' }}>
                                </div>
                                @if ($errors->has('archivoEntega'))
                                    <span class="text-danger">{{ $errors->first('archivoEntega') }}</span>
                                @endif
                                @php
                                    $filePath = public_path('entrega_tareas/' . $entregaTarea->archivoEntega);
                                    $fileExists = file_exists($filePath) && !empty($entregaTarea->archivoEntega);
                                    $extension = pathinfo($entregaTarea->archivoEntega, PATHINFO_EXTENSION); // Obtener la extensión del archivo
                                @endphp
                                @if ($fileExists)
                                    <a href="{{ asset('entrega_tareas/' . $entregaTarea->archivoEntega) }}"
                                        class="{{ $extension == 'pdf' ? 'text-danger' : ($extension == 'doc' ? 'text-primary' : 'text-primary') }}"
                                        target="_blank">
                                        <i
                                            class="fa {{ $extension == 'pdf' ? 'fa-file-pdf' : ($extension == 'doc' ? 'fa-file-word' : 'fa-file') }}"></i>
                                        {{ $entregaTarea->archivoEntega }}
                                    </a>
                                @endif
                            </div>
                            <div class="col-md-8 col-12">
                                <div class="form-group">
                                    ...
                                    <textarea name="comentarioEstudiante" rows="2" class="form-control" {{ $isHora ? '' : 'disabled' }}>{{ old('comentarioEstudiante', $entregaTarea->comentarioEstudiante) }} </textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-footer">
                        <div class="col-12 text-center">
                            ...
                            <button type="submit" class="btn btn-primary"
                                {{ $isHora ? '' : 'disabled' }}>Actualizar</button>
                        </div>
                    </div>
                </div>
            </form>
